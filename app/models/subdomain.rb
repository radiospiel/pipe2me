...
# - a subdomain has a ssl identity in *ssh_public_key*. This is generated by
#   the client and submitted during tunnel setup.
# - a subdomain has an *openssl_certificate*. This is generated by
...
module Subdomain::Etest
  ...
  def test_token_encryption
    ...
    assert_not_equal Subdomain.encrypt_token("foo"), Subdomain.encrypt_token("bar")
    assert_not_equal Subdomain.encrypt_token("foo"), "foo"
  end

  def test_token_is_stored_encrypted
    subdomain = self.subdomain protocols: %w(http https), token: "token"

    encrypted_token = subdomain.token
    assert_not_equal encrypted_token, "token"

    assert_equal subdomain, Subdomain.where(token: encrypted_token).first
    assert_nil Subdomain.where(token: "token").first
  end
end
